...
public class PartnerIdentity extends TestBase {
    ...
    public void getPartnerContextRecordIDOfPartnerPortal() {
        response = restAssuredUtils.sendGetRequest(pprrestEndpoints.GET_Partner_ContextRecordID, hooks.cookie).then()
                .extract().response();
    ...
    public void updatePartnerIdentity(DataTable table) {
        ...
        for (int i = 0; i < data.size(); i++) {
            ...
            responseFromPUT = restAssuredUtils.sendPUTRequest(
                    pprrestEndpoints.PUT_Partner_Identity.replace("{Identity_ID}", commonSteps.getIdentityID(table)),
                    hooks.cookie,
                    payload).then().statusCode(Integer.parseInt(data.get(i).get(4))).extract().response();
    ...
    public void createPartnerIdentity(DataTable table) {
        ...
        for (int i = 0; i < data.size(); i++) {
            ...
            responseFromPUT = restAssuredUtils.sendPostRequest(pprrestEndpoints.POST_Partner_Identities,
                    hooks.cookie,
                    payload).then().statusCode(Integer.parseInt(data.get(i).get(2))).extract().response();
    ...
    public void getIdentityWithIdentityTypeAsIdentityValueAsAndStatusCodeAs(String idType, String idValue,
            String status) {
        String identityID;
        String identityType;
        ...
        responseFromGET = restAssuredUtils
                .sendGetRequest(pprrestEndpoints.GET_Specific_Partner_Identity.replace("{identity_ID}", idValue)
                        .replace("{partner_id}", contextRecordID), hooks.cookie)
                .then().statusCode(200).extract().response();
        identityID = responseFromGET.then().extract().response().jsonPath()
                .getString("platform.record[0].identity_value");
        identityType = responseFromGET.then().extract().response().jsonPath()
                .getString("platform.record[0].record_locator");
        ...
        if (Integer.parseInt(status) == 200) {
            ...
            // since for 400 response we are getting xml as response instead of json so if
            // block is created
        }
    }
    ...
    public void deletePartnerIdentity(DataTable table) {
        ...
        for (int i = 0; i < data.size(); i++) {
            if (commonSteps.getIdentityID(table) != null) {
                responseFromDELETE = restAssuredUtils
                        .sendDeleteRequest(
                                pprrestEndpoints.DELETE_Partner_Identity.replace("{Identity_ID}",
                                        commonSteps.getIdentityID(table)),
                                hooks.cookie)
                        .then().statusCode(Integer.parseInt(data.get(i).get(2))).extract().response();
            } else {
                responseFromDELETE = restAssuredUtils
                        .sendDeleteRequest(
                                pprrestEndpoints.DELETE_Partner_Identity.replace("{Identity_ID}", "64674664"),
                                hooks.cookie)
                        .then().statusCode(Integer.parseInt(data.get(i).get(2))).extract().response();
            }
    ...
    public void approveAllTheAssetsInBB() {
        ...
        // responseFromGET =
        // restAssuredUtils.ExecuteAPI(RestAssuredUtils.HTTPMethod.GET, URI,
        // LoadEnvironment.B2BUser,
        // LoadEnvironment.B2BPwd, "", "", "");
        ...
        String ActivitykeyID = (responseFromGET.then().extract().response().jsonPath()
                .getString("connections[" + recordIDActivityIndex + "].activityKeys").split(":"))[0].substring(1);
        String latestID = (responseFromGET.then().extract().response().jsonPath()
                .getString("connections[" + recordIDActivityIndex + "].activityKeys." + ActivitykeyID + ".latestId"));
        String ids = responseFromGET.then().extract().response().jsonPath()
                .getString("connections[" + recordIDActivityIndex + "].activityKeys." + ActivitykeyID + ".ids");
        String action = responseFromGET.then().extract().response().jsonPath()
                .getString("connections[" + recordIDActivityIndex + "].activityKeys." + ActivitykeyID + ".action");

        String ActivityPayload = "{\"activityKeys\":{" + "\"" + ActivitykeyID + "\"" + ": {\"latestId\":" + latestID
                + ",\"ids\":" + ids + ",\"action\":" + "\"" + action + "\"" + "}}}";

        URI = pprrestEndpoints.b2bActivities.replace("{contextrecordID}", contextRecordID);

        // responseFromPOST =
        // restAssuredUtils.ExecuteAPI(RestAssuredUtils.HTTPMethod.POST, URI,
        // LoadEnvironment.B2BUser,
        // LoadEnvironment.B2BPwd, ActivityPayload, "", "");
        ...
        String reviewPayload = "{\"action\": \"Approve\",\"comment\": \"automation ids are approved\","
                + activityResponse.substring(1, activityResponse.length() - 1) + ","
                + ActivityPayload.substring(1, ActivityPayload.length() - 1) + "}";
        URI = pprrestEndpoints.b2bReview.replace("{contextrecordID}", contextRecordID);
        // responseFromPOST =
        // restAssuredUtils.ExecuteAPI(RestAssuredUtils.HTTPMethod.POST, URI,
        // LoadEnvironment.B2BUser,
        // LoadEnvironment.B2BPwd, reviewPayload, "", "");
