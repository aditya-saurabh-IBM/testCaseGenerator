...
//        }
        ...
        switch (methodType) {
            case GET: {
                response = RestAssured.given().urlEncodingEnabled(false).header("x-csrf-token", csrfValue).header("Accept", "application/json").log().all().cookies(cookies1).get(URI);
                break;
            }
            case POST: {
    ...
    private String getB2BSessionID(String userName, String password) {
        ...
        response = RestAssured.given().urlEncodingEnabled(false).header("Accept", "application/json").log().all().redirects().follow(false).get(baseURI + "/b2b/");
        /*loadBalancerCookie = APIResponseProvider.getCookies(loadBalancerCookieName, response);*/
        ...
        response = RestAssured.given().urlEncodingEnabled(false).redirects().follow(false).header("Accept", "application/json").log().all().cookies(cookies).get(LoginURI2);
        cookies = response.getCookies();
        ...
        //String redirectCookie1 = response.getCookies().get(1).toString().split(";")[0] + ";" + response.getCookies().get(0).toString().split(";")[0];
        String RedirectURI1 = "https://" + LoadEnvironment.IDM + "/auth/realms/" + LoadEnvironment.tname + "/login-actions/authenticate?session_code=" + sessionCode + "&execution=" + execution + "&client_id=b2b&tab_id=" + tabId;
        System.out.println(RedirectURI1);
//        MultivaluedMap<String, String> formData = new MultivaluedMapImpl();
//        formData.add("username", userName);
//        formData.add("password", password);
        // response = RestAssured.given().contentType("multipart/form-data").multiPart("username", userName).multiPart("password", password).redirects().follow(false).accept("application/json").header("Accept","application/json").log().all().cookies(cookies).post(RedirectURI1);
        response = RestAssured.given().contentType("application/x-www-form-urlencoded").formParam("username", userName).formParam("password", password).redirects().follow(false).accept("application/json").header("Accept", "application/json").log().all().cookies(cookies).post(RedirectURI1);
        cookies = response.getCookies();
        String RedirectURI2 = response.getHeader("Location");
        System.out.println("RedirectURI2 : "+ RedirectURI2);

        response = RestAssured.given().contentType("application/json").redirects().follow(false).accept("application/json").header("Accept", "application/json").header("Host",LoadEnvironment.host).log().all().cookies(cookies1).get(RedirectURI2);
        cookies = response.getCookies();
        ...
        response = RestAssured.given().contentType("application/json").redirects().follow(false).accept("application/json").header("Accept", "application/json").header("Host",LoadEnvironment.host).log().all().cookies(cookies1).get(RedirectURIb2b2);
    ...
    private String getCSRFToken() {
        String baseURI = LoadEnvironment.tenant;
        response = RestAssured.given().contentType("application/json").accept("application/json").header("Accept", "application/json").log().all().cookies(cookies1).get(baseURI + "/b2b/isSessionActive");
...
//    public static Response sendPutRequest(String endpoint, LinkedHashMap cookie, Object body) {
//
//        if (body.toString().trim().isEmpty()) {
